...
| F55 | The owner can see everything they can edit | Given a signed-in owner, when they switch to «Правка», then every editable region on the page shows its control at once and keeps showing it, with no pointer anywhere; when they switch to «Просмотр», then the page is the page a visitor reads — no edit control, no upload zone, no owner-only mark (draft chip, undescribed rim, reorder hint) and no unpublished card or unfinished tile is rendered at all; the choice survives a reload without a flash of the wrong state *(reformulated by I4 — the hover-reveal this originally sat on top of is gone, ADR-028; acceptance extended by I5, which is where «the page a visitor reads» stopped being true of three selector families only, ADR-032)* |
...
| F62 | The owner can see what needs them without touring the site | Given a signed-in owner, when they open the cabinet, then one room lists their drafts, their unpublished albums and projects and every photograph that failed to process — with the retry that already exists — each linking to the page that edits it; no room is ever indexed, and to anyone without a session no address of the cabinet exists *(I5: photographs with no description were removed from this list — a missing description is a hint in the album, not a task in the cabinet, ADR-036)* |

### Added by iteration I5 — authoring

Added 2026-08-16 from an owner request. Intake, impact map and exit criteria:
`docs/iterations/I5-authoring.md`. F63 is the only requirement in this document that lets a byte
leave this site's own origin, and it does so only after the reader asks for it.
